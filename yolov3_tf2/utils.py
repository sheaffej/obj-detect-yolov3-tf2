...
def draw_outputs(img, outputs, class_names, scale_font: bool = False):
    boxes, objectness, classes, nums = outputs
    boxes, objectness, classes, nums = boxes[0], objectness[0], classes[0], nums[0]
    wh = np.flip(img.shape[0:2])
    font_scale = 1
    if scale_font:
        font_scale = img.shape[0] // 800
    ...
    for i in range(nums):
        ...
        img = cv2.putText(img, '{} {:.4f}'.format(
            ...
            x1y1, cv2.FONT_HERSHEY_COMPLEX_SMALL, font_scale, (0, 0, 255), font_scale)
    return img
